...
def test_the_3d_view_is_built_once_and_kept_beside_the_snapshot(tmp_path, kernel, project, sketches, monkeypatch):
    ...
    # The lofted mesh is the one the view shows, so it is the one kept; the
    # exact staircase gets its own file only once someone asks for it.
    sidecar = path.with_name(path.name + slab_module.MESH_SIDECAR[True])
    assert sidecar.is_file()
    assert not path.with_name(path.name + slab_module.MESH_SIDECAR[False]).exists()
    kernel.surfaces(state, project=project, loft=False)
    assert path.with_name(path.name + slab_module.MESH_SIDECAR[False]).is_file()
...
def _mesa_with_films(kernel, *, films=("Al2O3",), thickness=0.010, second_mesa=False):
    """A tungsten mesa on the wafer under one conformal film per name."""
    materials = default_materials()
    project = ProjectDefinition(
        "Mesa", grid_dict(default_grid()), kernel="slab", resolution_um=0.001, resolution_xy_um=0.001
    )
    sketches = {
        "mesa": QuickSketch("mesa", [SketchShape("rectangle", parameters={"center": (-0.3, 0.0), "size": (0.3, 0.3)})]),
        "low": QuickSketch("low", [SketchShape("rectangle", parameters={"center": (0.3, 0.0), "size": (0.3, 0.3)})]),
    }
    ...
    if second_mesa:
        # A second mesa beside the first, planar-filled to another height,
        # so the two shoulders' bands sit at different planes.
        state = run(state, step(ProcessType.DEPOSIT, mask_source="quick_sketch", keep="inside",
                                parameters={"target": 0.021, "mode": "planar", "sketch_id": "low"}, output_material="W"))
    for name in films:
        state = run(state, step(ProcessType.DEPOSIT, parameters={"target": thickness, "mode": "conformal"}, output_material=name))
    return state, project, materials
...
def _slanted(mesh):
    ...
    return int(((np.abs(normals[:, 2]) > 0.05) & (np.abs(normals[:, 2]) < 0.95)).sum())


def test_the_3d_view_lofts_the_sampled_bands_into_a_closed_surface(kernel):
    """A film's shoulder is stored as bands one z step tall. The 3D view
    joins each band's outline to the next with slanted faces, at the bands'
    mid heights, so the shoulder is a slope instead of a staircase; the
    result is still a closed surface with the stored volume, and the exact
    staircase is still there for the asking. The mesa under the film, whose
    walls are not sampled, stays vertical either way."""

    from deviceflow._internal.mesh.builder import build_material_meshes

    state, _project, _materials = _mesa_with_films(kernel, films=("Al2O3", "TiN"))
    geometry = state.device._state
    step_um = state.device.conformal_resolution
    exact = build_material_meshes(geometry, manifold=False)
    lofted = build_material_meshes(geometry, manifold=False, loft=2 * step_um, loft_reach=8 * step_um)
    for material, mesh in lofted.items():
        assert mesh.is_watertight and mesh.is_winding_consistent, material.name
        assert mesh.volume == pytest.approx(exact[material].volume, rel=1e-3), material.name
        if material.name in ("Al2O3", "TiN"):
            assert _slanted(mesh) > 0 and _slanted(exact[material]) == 0, material.name
            assert len(mesh.faces) < len(exact[material].faces), material.name
        else:
            assert _slanted(mesh) == 0, material.name
    # The two films share their interface: the outer film's interface faces,
    # slanted ones included, all lie on the inner film's surface, so the
    # viewer can leave them out while both films are shown.
    inner = lofted[[m for m in lofted if m.name == "Al2O3"][0]]
    outer = lofted[[m for m in lofted if m.name == "TiN"][0]]
    against_inner = outer.metadata["interface_faces"]
    assert against_inner.any() and not against_inner.all()
    # Both films' rings are the same arrangement nodes at the same mid
    # heights, so every corner of such a face is a vertex of the inner film.
    inner_vertices = set(map(tuple, inner.vertices.round(9).tolist()))
    corners = outer.triangles[against_inner].reshape(-1, 3).round(9)
    assert all(tuple(corner) in inner_vertices for corner in corners.tolist())
    normals = outer.face_normals[against_inner]
    assert ((abs(normals[:, 2]) > 0.05) & (abs(normals[:, 2]) < 0.95)).any()


def test_each_feature_is_lofted_on_its_own(kernel):
    """Two mesas of different heights under one film: their shoulders are
    at different heights, so at most planes one film ring changes while
    another does not. Each ring is joined to its own continuation, so both
    shoulders slope, in 3D and in the section, and the mesh stays closed.
    Before, a change anywhere in a plane kept the whole plane stepped."""
    from deviceflow._internal.mesh.builder import build_material_meshes
    from process_studio.kernels.slab import _smooth_section_shapes, resolution_um

    state, project, _materials = _mesa_with_films(kernel, second_mesa=True)
    geometry = state.device._state
    step_um = state.device.conformal_resolution
    lofted = build_material_meshes(geometry, manifold=False, loft=2 * step_um, loft_reach=8 * step_um)
    film = lofted[[m for m in lofted if m.name == "Al2O3"][0]]
    assert film.is_watertight and film.is_winding_consistent
    normals = film.face_normals
    slanted = (abs(normals[:, 2]) > 0.05) & (abs(normals[:, 2]) < 0.95)
    centres = film.triangles_center[slanted]
    # slanted faces on both shoulders (x < 0 and x > 0), which top out at different heights
    assert (centres[:, 0] < 0).any() and (centres[:, 0] > 0).any()
    assert abs(centres[centres[:, 0] < 0][:, 2].max() - centres[centres[:, 0] > 0][:, 2].max()) > 0.01

    section = state.device.cross_section((-0.8, 0.0), (0.8, 0.0))
    shapes = _smooth_section_shapes(section, state, resolution_um(project))
    slanted_at = []
    for rings, name in shapes:
        ...
        for ring in rings:
            for (x0, z0), (x1, z1) in zip(ring, ring[1:] + ring[:1]):
                if abs(x1 - x0) > 1e-9 and abs(z1 - z0) > 1e-9:
                    slanted_at.append(0.5 * (x0 + x1))
    # distances along the cut: the high mesa spans 0.35..0.65, the low one 1.15..1.45
    assert any(x < 0.8 for x in slanted_at) and any(x > 0.8 for x in slanted_at)


def test_a_floor_keeps_its_flat_top_when_lofting(kernel):
    """The band of film on the floor beside a mesa and the sidewall band
    above it overlap well, but the floor's outline is far from the
    sidewall's; joining them would tilt the whole floor. Such a pair is
    left as stored: the floor film keeps a horizontal top."""
    import numpy as np

    from deviceflow._internal.mesh.builder import build_material_meshes

    # A film thinner than two z steps, so even the floor band is "thin".
    state, _project, _materials = _mesa_with_films(kernel, thickness=0.0015)
    geometry = state.device._state
    step_um = state.device.conformal_resolution
    lofted = build_material_meshes(geometry, manifold=False, loft=2 * step_um, loft_reach=8 * step_um)
    film = lofted[[m for m in lofted if m.name == "Al2O3"][0]]
    assert film.is_watertight
    normals = film.face_normals
    centres = film.triangles_center
    # horizontal, up-facing faces away from the mesa, at the floor film's top height
    away = (centres[:, 0] > 0.0) & (normals[:, 2] > 0.999)
    assert away.any()
    assert np.allclose(centres[away][:, 2], 0.8 + 0.0015, atol=1e-6)
