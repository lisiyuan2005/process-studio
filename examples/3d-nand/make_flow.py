...
The block is four ON pairs tall and sits between two slits, with two channel
holes, a two-step staircase and word-line contacts. Dimensions are a few
...
# Sizes are chosen so the flow runs in minutes: the two isotropic etches
# (sacrificial SiN removal, W etch-back) cost time in proportion to how
# far the front must creep and how much void surface it creeps from, so
# the block is narrow and has two holes. Scale up here if you have time.
PAIRS = 4
...
steps += [
    ...
    etch("SiN removal (hot H3PO4)", REACH, {"SiN": 1}, directional=0.0, tool="Wet bench"),
...
sketches = {
    ...
    "slits": {"name": "Slits", "shapes": [rectangle(x, 0.0, 0.12, 0.4) for x in SLITS]},
...
flow = {
    ...
    "section_lines": [
        {"name": "Through holes and slit", "start": [X0, 0.0], "end": [X1, 0.0]},
        {"name": "Along the slit", "start": [-0.04, -0.15], "end": [-0.04, 0.15]},
