...
结构按真实流程排列，尺寸比量产器件粗几倍，块做得很窄，让两步各向同性刻蚀在一分钟内算完（全部 34 步细节版约 56 s，简化版约 29 s）：
...
- 各向同性去 SiN 和 W 回刻是整条流程里最慢的两步（细节版约 20 s 和 11 s，简化版 4 s 和 5 s）：每一步都要把上一步新开出的空腔表面向内推一小段（步长是最薄屏障层的一半），推得越远、孔越多越慢，所以示例只放两个孔。
