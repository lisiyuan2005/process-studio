...
| `view section --step STEP [--axis x\|y --at UM \| --line X0 Y0 X1 Y1] -o cut.png` | 截面：`.svg` 写矢量轮廓（每种材料一条填充路径，放大不糊），其他扩展名写 PNG |
| `view top --step STEP [--no-steps] [--hide MATERIAL]... -o top.png` | 俯视图（同样 `.svg` 是矢量、其他是 PNG），`--hide` 透过某种材料看它下面（可重复），按每点最上层材料的颜色画；同一材料自己的高度分界默认画一条压暗的线，`--no-steps` 关掉 |
