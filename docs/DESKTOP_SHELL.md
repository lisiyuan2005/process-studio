...
src/deviceflow              并入的 DeviceFlow 0.2.0 几何内核，未改动（见 src/deviceflow/VENDOR.md）
```
...
| `process_studio_snapshots/` | 每步一个状态存档（`.dfz`，DeviceFlow 自己的格式）；旁边的 `*.mesh*.npz` 是这一步的 3D 显示网格，第一次构建后留下来，删掉只会让下次打开 3D 视图重新构建 |
...
| 方法 | 作用 |
| --- | --- |
| `ping` / `describe` | 版本与能力（可用内核、工艺类型、掩膜来源、显示插值上限、命令行路径） |
| `create_workspace` / `open_workspace` | 建立或打开工作目录，返回完整 document |
| `save_document` | 写回工程、分支、步骤、Recipe、材料 |
| `create_branch` / `rename_branch` / `delete_branch` | 工艺分叉：在某一步之后分出新分支（连同已算好的结果），改名，删除 |
| `plan_grid` | 目标分辨率和窗口会不会真的改动什么（`unchanged`）——应用要丢弃全部结果，所以值得先问 |
| `set_grid` | 改几何分辨率，可同时改工程窗口；都会删除全部已存结果 |
...
只有一个内核：slab（并入的 DeviceFlow）。视图、执行和快照格式都由内核自己提供，`runner.py` 和 `protocol.py` 不判断内核 id，`describe` 照旧报告有哪些内核可用——工程记着自己用的是哪个，`save_document` 拒绝改动它。原来的 level set 内核在 `0.9.8` 之后删掉了（代码在 `archive/levelset` 分支），用它建的工程打开时被明确拒绝并说明去哪个版本打开。内核能做什么见[内核说明](KERNELS.md)。

## 精度控制

slab 工程有三档**膜模型**（fidelity），顶栏精度按钮旁边的按钮切换，**Voxel**（体素版，见 KERNELS.md：高度精确、平面按格子算，整个流程几秒钟，用来快速看结构）、**Detailed**（细节版）和 **Simplified**（简化版）：细节版的膜按球体膨胀算、沿 z 按分辨率采样，角是圆的，planar 的腔口上下沿都会长膜唇（像溅射那样把腔口收死）；简化版的膜全是直角：每个高度上固体轮廓向外推 t，露出的水平面（顶面、保形时还有底面）各移 t 并向外多伸 t 盖住侧壁膜，膜在叠层各平面及其 ±t 之间是常数，每段只算一次多边形偏移，窄于 2t 的缝一样会被封死（pinch-off 保留）；各向同性刻蚀在简化版里前沿也是方的（空腔按盒子而不是球体长大，每个平面一段采样），屏障分步照旧。3D NAND 示例：细节版 56 s，简化版 29 s（板层归一改成只检查改动过的板层之后；之前是 94 s 和 51 s）。各档的结果分开存（简化版和体素版的快照和摘要用带 `~simplified`、`~voxel` 后缀的步骤键），切换只是换成看另一档已算好的结果，步骤状态随之变化，不会丢掉任何一档；RPC 里是 `project.fidelity`，命令行 `fidelity [detailed|simplified|voxel]`，流程文件 `fidelity`。新工程默认**简化版**。

另有一种步骤类型 **Flip wafer**：把整片翻过来，接下来的步骤做在原来的背面——双面集成就是这么做的。没有掩膜、没有材料、没有速率，只有一个 `axis`：绕 **y** 轴翻（默认）横向就镜像 x，绕 **x** 轴翻则镜像 y。**体积逐位不变**，翻两次回到原样。镜像是认真的：翻过来的东西本来就左右反了，不镜像的话背面掩膜和正面图形对不上；掩膜永远按"你现在看到的这一面"读，所以翻完再画的 sketch 落在画的地方。背面长了膜之后再翻回来，正面会整体抬高一个膜厚——晶圆背面多了一层，放回卡盘上就是高了。

另有一种步骤类型 **Oxidation**：列出会氧化的材料和相对速率，给被消耗的厚度（或时间），露出的表面向内这么厚的一层原地变成指定的氧化物（默认 SiO₂）。前沿和湿法刻蚀完全一样（同样从暴露面出发、同样的屏障分步、简化版同样是方前沿），只是刻掉的部分变成氧化物填回去，体积守恒、外形不变；真实氧化会膨胀 2.2 倍，这里刻意不算。RPC/流程文件的类型名是 `oxidation`，`outputMaterial`/`material` 是产物。

沉积有两种模式：**Conformal** 在每个露出的表面上长同样厚的膜（侧壁、底、顶都是 t，外凸角变圆）；**Planar** 是从正上方落下来的膜、侧壁也长：每个高度上，膜 = 该高度固体轮廓向外推 t（侧壁膜，在墙的上下边缘平着截止）加上天空能看见的水平面各抬高 t（顶面膜向外多伸 t 盖住下面的侧壁膜，凸角是直角），再减掉“上方整列有固体”的阴影。所以孔收窄 t、底升高、深度不变，台阶还是台阶，侧壁里横着挖进去的凹腔（悬垂下面）什么也落不到，腔口上沿（悬垂下沿）和下沿（墙顶）都平着截断，只有膜厚超过腔高时地面才会把腔口顶死。任何地方都不在 z 方向倒圆，膜在叠层各平面及其加 t 之间是常数，每段一个采样就是精确的，所以比保形沉积便宜得多；空片上的 Planar 是一整块平板（衬底就是这样做的）。填平台阶用 Planar 再 CMP：每级台阶的地面都升高 t，抛到目标高度后台阶间就是填充材料。slab 工程有两个分辨率：z 步长（保形沉积和各向同性刻蚀沿高度的采样步长，即圆角肩部台阶的高度）和 XY 弧线弦高（平面内圆角折线逼近的最大偏差，决定每层轮廓的顶点数；留空则跟随 z 步长）。两者独立：z 调细只增加板层数，XY 调细只增加每层顶点数。RPC 的 `plan_grid` / `set_grid` 用 `targetSpacingNm` 和可选的 `targetSpacingXyNm`，命令行是 `window --spacing NM --spacing-xy NM`（0 表示跟随 z），流程文件是 `resolution_nm` 和 `resolution_xy_nm`。
...
- **切换步骤时保持视角**：换一步会先把上一步的结果从屏幕上清掉（一个步骤的几何不能挂着另一个步骤的名字），所以 3D 画布是**卸载再重建**的，而新画布默认是那个三分之四视角。转过去的方向是用户自己选的，每换一步就要重新转一遍，正是把一条流程看下来变得难受的原因。所以相机的位置和 orbit 目标记在画布外面（`Viewport` 的一个 ref，`RememberCamera` 负责存取），下一块画布挂上来时放回去；存的时候连当时的模型尺度一起存，下一步的模型更高就按比例站远一点（`restoredCamera`，和 `KeepInView` 在挂载期间做的是同一件事）。截面图和俯视图的缩放平移同理，各记一份。去别的视图转一圈回来也保持。
- **Sampling**（视口底栏）：只影响显示。它提高截面图片的光栅密度，把同一份精确几何画得更细，不改变内核算出的结果（上限见 `describe` 的 `maximumInterpolation`）。
...
**顶栏的 Display 菜单**放的是「影响网格怎么建」的开关，不是「显示什么」：三角化方式（Ear clipping / Delaunay），以及是否构建**材料之间的界面面**。

voxel 工程的 Display 菜单还多一组 **Voxel 3D view**，两种 3D 画法（选择记在本机，按工程的 fidelity 生效）：

- **Cells**：每个细格的边界各一个面（原来的画法），细格比显示精度还细时先按 2 的幂合并。建得快（3D AND 示例 0.1 nm 最后一步约 3 s），但 0.1 nm 下看到的其实是约 0.8 nm 的格子，三角形多（266,164 个）。
- **Polygons**：每层板把各材料的细格和斜切线轮廓（和俯视图同一套）拼成一张平面剖分，一起简化到 0.1 个细格以内（共用的边仍是一条，不会裂缝、重叠），再交给 slab 内核的网格构建器。全分辨率、体积与状态一致，三角形少得多（同一例 58,542 个），但第一次建较慢（约 10–15 s，内容相同的板只算一次、多线程），之后缓存。
...
这一整套是**加速器，不是依赖**：起不了进程、子进程死了、子进程读不到交接文件，任何一种情况都退回本进程顺序建，并且从此不再问。`PROCESS_STUDIO_MESH_WORKERS=1` 可以直接关掉它，命令行 `process-studio cores` 会告诉你这台机器上池能不能起来、用几个核——打包脚本就是拿这条命令做冒烟测试的，因为冻结的可执行文件要靠启动**它自己**来开子进程，这是唯一一件在源码里对、在打包后可能丢的事。`cores` 还报一个 `warmed`：子进程里几何库（shapely/trimesh）预热成功才是 true，false 说明打包丢了东西，脚本会让构建失败。
...
- **俯视图的图例也是开关**，但含义不同：那张图是 worker 画出来的 PNG，所以隐藏一种材料是**重新画一张**——被隐藏的材料既不画，也不再遮住它下面的东西（`get_top_view` 的 `hidden: [名字]`）。一层 blanket 膜盖满整个窗口时，俯视图就是一块纯色；把它看穿，才看得见它盖住的图形和台阶。内核在自顶向下认领可见区域时跳过这些材料（`TopView(state, materials, hidden)`）。台阶线也跟着重画：量的是剩下那些东西的高度。隐藏状态和 3D 视图共用一份，切过去就还是隐藏着的。
...
怎么知道一个工作目录是不是"从别处来的"：库有一个 id，工作目录记下它上次抄的是哪个库（`settings` 表）。

**删除只按明说的删**：保存文档时，文档里少了哪一项**不算删除**，只有 `document.deleted`（`{"materials": [id…], "tools": […], "recipes": […]}`）里点名的才删——界面上点删除、CLI 的 `materials rm` / `tools rm` 都会填它。以前是"文档里没有、副本里有，就当用户删了"，结果任何一份落后或残缺的文档（没刷新的标签页、同时开着的第二个应用、升级前后的窗口）一次自动保存就能把库里的东西从所有工程里删掉，而且再打开工程也拿不回来。

**库记得用户删过什么**（`deleted` 表，按名字）。所以同一个库的副本现在也会往回读，但只读回**库里没有、又不是用户删掉的**东西：库因为任何别的原因丢了的材料、recipe、工具，打开任何一个还留着副本的工程就回来；用户明确删掉的不会被旧副本交回来。重新建一个同名的，删除记录就消掉。

副本在**打开工作目录时**刷新成库的样子（`refresh_library_copy`），保存时也跟着写。

**新建工程不再重新播种**：`default_materials()` / `default_recipes()` / `default_tools()` 只在库是空的时候写一次（第一次装）。否则每开一个新工程都会把出厂的 Si 盖回你改过的那个，把你删掉的 recipe 全放回来。
...
一条流程走到某一步，想试两种做法——厚一点的氧化层、换一种刻蚀——这就是一次分叉：**分叉点之前是同一条流程，之后各走各的**。

**入口有三个**：步骤右键菜单的 **Fork a branch here…**（最直接：在哪一步右键就从哪一步分）、顶栏 **Branch** 菜单（列出所有分支可切换，加上 Fork after 当前选中步 / Rename / Delete），命令行 `branches add NAME [--after STEP]`。顶栏原来那个分支下拉仍然只管切换。

**分叉不重算**：新分支复制分叉点及之前的步骤，并把这些步骤**已经算好的快照重新挂到新分支上**——同样的步骤就是同样的结果。快照文件是**共享**的（`branch_snapshots` 按「分支+步骤」记，快照本身引用计数），所以分叉既不占空间也不花时间，只有分叉点之后新加的步骤要跑。摘要（digest）也一并抄过去（`DigestCache.copy`），否则新分支的结果虽然在、却因为没有摘要为它背书而全部显示成 stale，整条流程会从第一步重跑一遍来证明它已经知道的事。

**删除**只删这条分支独有的东西：快照文件只有在**没有任何分支再指向它**的时候才真的删掉，所以删掉一条分支不会动它和母分支共用的那一段。两条护栏：工程至少保留一条分支；**有别的分支从它分出去的分支不能删**（要先删子分支），否则子分支的来历就断了。

**名字**必须非空且在工程内唯一——分支下拉里显示的就是名字，两条同名的只能靠位置区分。

**看得出来分叉**：步骤卡副标题上会出现一个 `⑂ 分支名`（多条时是 `⑂ N branches`）的小标记，因为只显示一条分支的列表本来说不出「流程在这里还往别处走」。
...
`npm run dev` 只启动浏览器预览：它有完整界面但没有 worker，运行和视图都会明确报错，不会伪造结果。
...
- **Windows**（`build_desktop.ps1`）：不用 PyInstaller。脚本自己下载官方的 embeddable Python（`PROCESS_STUDIO_EMBED_PYTHON` 指定版本，默认 3.12.7）解到 `desktop/src-tauri/resources/python`，改它的 `._pth` 打开 `import site` 并加上 `Lib\site-packages`，用 `get-pip.py` 装上 pip，再装包——全是 PyPI 上的普通 wheel，不需要系统装 Python。外壳找不到 `resources/worker` 下的可执行文件时，会去找 `resources/python/python.exe` 并以 `-m process_studio.worker` 启动它（见下面「杀毒软件」一节的原因）。两条路径做完都会做一次 `describe` 冒烟测试，再执行 `npm run tauri build`。发布版通过 `PROCESS_STUDIO_WORKER` 可以覆盖 worker 路径（两个平台都适用）。
  ...
  **Windows 包装的是明确列出的依赖集**：`numpy pillow gdstk openpyxl pyyaml shapely trimesh mapbox-earcut truststore certifi`，再用 `--no-deps` 装 `process_studio` 本身。`scikit-fmm` 和 `scikit-image`（连带 `networkx`、`imageio`、`tifffile`、`lazy-loader`）是 level set 内核专用的求解器和 marching cubes，随它一起不装了；PyInstaller 的 spec 也把 `scipy`、`skfmm`、`skimage` 写进 `excludes`。`scipy` 也不装了：唯一用到它的是 trimesh 把面的颜色转成顶点颜色（一个稀疏矩阵）来写 mesh 文件，而导出现在直接给顶点上色——同一张图，少一个几十 MB 的依赖。`tests/test_packaging.py` 用一个会 ImportError 的假 scipy 在**新进程**里真导一次 mesh 把这件事钉住：trimesh 是在 import 时决定 scipy 在不在的，在已经 import 过的进程里再屏蔽证明不了什么（这正是本地绿、CI 红的原因）。`matplotlib` 早已从核心依赖里删掉（deviceflow 自己的 PNG 导出是惰性 import 加优雅降级）。装完之后脚本还会删掉 `pip`/`setuptools`/`wheel`（构建期工具，运行时从不 import）、每个依赖包自带的 `tests`/`test` 目录和 `__pycache__`。这一套让 Windows 包从 `v0.9.0` 的 139 MB（解压 354 MB）降到约 65-70 MB。

**发布与检查更新**：一个版本一份产物，两个平台各一个：`ProcessStudio-Windows.zip` 和 `ProcessStudio-macOS.zip`（外加 Windows 安装程序 `ProcessStudio-Setup.exe` 和 dmg）。推一个 `v*` 标签（版本号同时写在 `pyproject.toml`、`src/process_studio/__init__.py`、`desktop/package.json`、`desktop/src-tauri/tauri.conf.json` 和 `Cargo.toml` 里）会构建两个平台并由 `publish` 任务发成一个 GitHub Release；也可以手动触发工作流并在 `release` 框里填 tag。首页版本号旁边的 **Check for updates** 走 worker 的 `check_update`（读 `releases/latest`，页面本身不联网），给出本平台资产的 **Download** 按钮和 **Release notes**，两者都由 worker 的 `open_url` 用系统浏览器打开，且只允许仓库自己的地址。

**版本号从 `0.9.8` 重置成 `0.1.0`**（只剩一个内核、一个版本，从这里重新数）。所以"有没有新版本"不能只看"更大"：`describe_release` 同时给出 `available`（最新 release 与当前构建**不同**）和 `newer`（严格更大），界面用前者，`0.9.8` 的用户因此仍然会被告知有 `0.1.0` 可装。

`Desktop builds` 工作流跑的就是这两个脚本，每个平台一个作业；手动触发时可以在 GitHub 的 Run workflow 对话框里选平台。构建前先执行 `python -m pytest -q`，前端测试由脚本里的 `npm run test` 负责。Windows 用 `--bundles nsis`，交付 `ProcessStudio.exe` 加同级的 `resources/`（免安装版，应用内更新装的也是它），外加一个 NSIS 安装程序 `ProcessStudio-Setup.exe`：按当前用户安装（不需要管理员，安装目录用户自己可写，应用内更新才能覆盖），把 embeddable Python 作为资源装到 exe 同级的 `resources/python`，和免安装版目录结构一样；主程序名用 `mainBinaryName` 固定成 `ProcessStudio`。macOS 用 `--bundles app`，交付 `Process Studio.app` 和 DMG。不生成 MSI。
...
**杀毒软件把 Windows 的 worker 删了（`v0.9.0` 之前的版本）**：`v0.8.x` 及更早的 Windows 包用 PyInstaller，杀毒或终端防护软件（Windows Defender、CrowdStrike 之类）经常把这类单文件可执行程序当可疑文件删掉，报 `The packaged process worker was not found`。`v0.9.0` 起 Windows 包不再用 PyInstaller，改成上面说的 embeddable Python + 普通 wheel，`python.exe` 是官方签名的解释器，安装的包都是 PyPI 正式发行的 wheel，不再是这类工具典型的误报目标。仍然遇到同样报错时：到 Windows 安全中心 → 病毒和威胁防护 → 保护历史记录里恢复并允许，或者把解压目录加入排除项，再重新解压一次；公司电脑可能要找 IT 放行。另一条路是不依赖打包的 worker：`pip install "process-studio @ git+https://github.com/lisiyuan2005/process-studio@main"`，发布版的外壳在找不到打包 worker 时会依次找 `PROCESS_STUDIO_WORKER`、`resources/python/python.exe`、PATH 和 pip 的 Scripts 目录（Windows 的 `%APPDATA%\Python\Python3xx\Scripts`、`%LOCALAPPDATA%\Programs\Python\Python3xx\Scripts`；macOS/Linux 的 `~/.local/bin`、`/opt/homebrew/bin`、`/usr/local/bin`）里的 `process-studio-worker`。

**「Could not check: certificate verify failed / unable to get local issuer certificate」**：这跟网络和 GitHub 都没关系，是打包版的 Python 不知道该信任谁。它自带的 OpenSSL 是在构建机上编译的，里面写死的 CA bundle 路径在用户机器上指向一个不存在的文件，于是任何 https 请求都验不过证书。`v0.9.5` 起更新检查显式构造验证上下文（`update.verifier()`）：优先用 **truststore** 读**操作系统自己的证书库**——那是这台机器的浏览器信任的同一套，因此公司 IT 为 https 检查代理装进去的根证书也在里面，这是任何我们自带的 bundle 都不可能知道的；读不到就退回自带的 **certifi**，再不行才用解释器自己的默认值。两个包都在依赖里，也在 Windows 的显式安装清单里。证书验证失败的报错也不再直接把 OpenSSL 那行原样抛给用户，而是说明这是信任库的问题、公司网络该怎么办、以及去发布页用浏览器下载总是可行。
