...
- 3D 视图底栏的材料图例是**显示开关**：点一下隐藏该材料的表面，再点一下显示，便于看内部结构。它只过滤已经取回的三角面，不重新计算。
- **俯视图的图例也是开关**，但含义不同：那张图是 worker 画出来的 PNG，所以隐藏一种材料是**重新画一张**——被隐藏的材料既不画，也不再遮住它下面的东西（`get_top_view` 的 `hidden: [名字]`）。一层 blanket 膜盖满整个窗口时，俯视图就是一块纯色；把它看穿，才看得见它盖住的图形和台阶。两个内核都支持：slab 内核在自顶向下认领可见区域时跳过这些材料（`TopView(state, materials, hidden)`），level set 内核取每列最上层**不属于**这些材料的节点。按高度着色时同样生效（量的是剩下那些东西的高度），此时图例是高度，所以旁边多一个「N hidden」按钮把隐藏的放回来。隐藏状态和 3D 视图共用一份，切过去就还是隐藏着的。
