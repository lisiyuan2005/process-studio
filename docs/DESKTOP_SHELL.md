...
- 截面工具栏的 **Smooth steps / Exact slabs** 只影响画法：slab 内核把保形膜按分辨率逐层采样存成板层，圆角肩部因此是台阶高等于分辨率的阶梯。默认把相邻薄层（厚度不超过两倍分辨率）在各自中高处的轮廓用斜边连起来，画出它们采样的那个光滑表面；厚层（衬底、平面膜、直刻的沟槽）保持竖直边，区间数变化的地方（pinch-off、分叉）按原样画。存储的几何一点不动，切到 Exact slabs 就看到原始阶梯。RPC 的 `get_section` 用 `smooth` 参数，命令行是 `view section --exact`。level set 内核的截面本来就是插值的，忽略这个开关。
