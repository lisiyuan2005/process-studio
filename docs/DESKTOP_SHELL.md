...
- **俯视图的图例也是开关**，但含义不同：那张图是 worker 画出来的 PNG，所以隐藏一种材料是**重新画一张**——被隐藏的材料既不画，也不再遮住它下面的东西（`get_top_view` 的 `hidden: [名字]`）。一层 blanket 膜盖满整个窗口时，俯视图就是一块纯色；把它看穿，才看得见它盖住的图形和台阶。两个内核都支持：slab 内核在自顶向下认领可见区域时跳过这些材料（`TopView(state, materials, hidden)`），level set 内核取每列最上层**不属于**这些材料的节点。台阶线也跟着重画：量的是剩下那些东西的高度。隐藏状态和 3D 视图共用一份，切过去就还是隐藏着的。
...
顶栏是一条菜单栏，按用途分组：**File**（新建/打开/最近/保存/另存为；导入流程文件、GDSII、材料/配方/工具库；导出流程为 Excel/CSV/流程文件、各库为 xlsx/csv、3D 表面；打开工作目录所在文件夹；关闭工程），**Edit**（撤销/重做、加步骤、复制/粘贴/重复/删除/全选/上下移动/跳过与恢复），**Branch**（切换分支、从选中步分叉、重命名、删除），**View**（3D/截面/俯视，俯视图的台阶线开关，日志），**Run**（运行、运行到选中步、停止、丢弃结果全部重跑、膜模型 Detailed/Simplified、精度、命令控制台），**Libraries**（材料、配方、工具），**Help**（文档、快捷键、检查更新、关于）。菜单右边是分支、精度、膜模型、内核徽标，最右是保存状态、Log 和 Run。
