...
顶栏是一条菜单栏，按用途分组：**File**（新建/打开/最近/保存/另存为；导入流程文件、GDSII、材料/配方/工具库；导出流程为 Excel/CSV/流程文件、各库为 xlsx/csv、3D 表面；打开工作目录所在文件夹；关闭工程），**Edit**（撤销/重做、加步骤、复制/粘贴/重复/删除/全选/上下移动/跳过与恢复），**View**（3D/截面/俯视，俯视图的台阶线开关，日志），**Run**（运行、运行到选中步、停止、丢弃结果全部重跑、膜模型 Detailed/Simplified、精度、命令控制台），**Libraries**（材料、配方、工具），**Help**（文档、快捷键、检查更新、关于）。菜单右边是分支、精度、膜模型、内核徽标，最右是保存状态、Log 和 Run。
